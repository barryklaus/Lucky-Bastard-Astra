...
const eyeY=-18+(a.eyes===2?3:0);for(let side=-1;side<=1;side+=2){const x=side===-1?-16:17,y=eyeY+(side===1?3:0),wide=a.eyes===4?1.25:a.eyes===3?.72:1; c.save();c.translate(x,y);c.scale(wide,1);const h=dead?.6:hurt?10:Math.max(1,7*(1-blink));P(c,`M-12 0 Q-8 ${-h-5} 0 ${-h-3} Q10 ${-h-3} 12 0 Q8 ${h} 0 ${h+1} Q-10 ${h}-12 0Z`,'#f6eed7',LB.ink,2);if(!dead&&blink<.85){let px=look*3+Math.sin(t*.7+side)*.5;if(dizzy)px=Math.sin(t*8+side)*5;E(c,px,0,4.7,h*.7,a.eyeColor,LB.ink,1);E(c,px+1,0,1.9,h*.56,LB.ink);E(c,px+1,-2,1,1,'#fff7dc')}
// A full-width upper lid overlays the entire upper half of the eyeball.
// Its lower edge remains readable during squints, asymmetric reactions and blinks.
const lidTilt=angry?(side<0?-2:2):smug?(side<0?1:-1):0,lidY=dead?1:lidTilt;
P(c,`M-12 ${lidY} Q-8 ${-h-5} 0 ${-h-3} Q10 ${-h-3} 12 ${lidY} L12 ${lidY+1} Q0 ${lidY+3}-12 ${lidY+1}Z`,skin,LB.ink,1.8);
P(c,`M-12 ${lidY} Q0 ${lidY+3} 12 ${lidY}`,null,LB.shade(skin,-55),1.6);P(c,'M-10 9 Q0 13 9 8',null,LB.shade(skin,-40),1.4);let browY=hurt?-15:-12;P(c,`M-12 ${browY+((angry&&side<0)?-3:1)} Q0 ${browY-4} 12 ${browY+(angry?6:0)} L11 ${browY+4} Q-3 ${browY-1}-12 ${browY+4}Z`,a.hairColor,LB.ink,1.3);c.restore()}
