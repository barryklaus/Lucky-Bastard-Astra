...
const crowdUpdate=L.Crowd.update.bind(L.Crowd);L.Crowd.update=noop;L.Crowd.react=noop;L.Dialogue.update=noop;L.UI.hud=noop;L.UI.announce=noop;L.UI.showCombat=noop;L.UI.rewards=noop;L.UI.results=noop;L.UI.trivia=noop;
...
test('Weapon-specific finishers select distinct outcomes and artwork',()=>{const cases=[[2,'panPancake','pancake'],[8,'cueImpale','impale'],[15,'chairFold','folded'],[36,'laserSplit','laserSplit']],cv=createCanvas(1840,500),c=cv.getContext('2d');c.fillStyle='#e3d7b5';c.fillRect(0,0,1840,500);for(let i=0;i<cases.length;i++){const[id,key,effect]=cases[i],[p,e]=setup(400+i);L.Particles.items=[];L.Particles.words=[];L.Particles.stains=[];p.x=1100;e.x=1270;p.weapon=id;e.hp=0;L.Combat.lethal(p,e,{kind:'weapon',rare:true});assert.equal(L.game.finalHold.finisher,key);assert.equal(e.finisherEffect.kind,effect);if(key==='laserSplit')assert(e.upperDetached);if(effect==='impale')assert.equal(p.weapon,0);tick(30);c.save();c.beginPath();c.rect(i*460,0,460,500);c.clip();c.translate(i*460-900,0);L.Rig.draw(c,p,L.game.time);L.Rig.draw(c,e,L.game.time);L.Physics.draw(c);L.Particles.draw(c);c.restore();L.text(c,L.weaponById(id).name.toUpperCase(),i*460+230,35,18,L.ink,'Arial','center')}fs.writeFileSync(path.join(ROOT,'tests/weapon-finishers-render.png'),cv.toBuffer('image/png'))});

test('Three-hit finishers trigger momentum camera and crowd eruption',()=>{const[p,e]=setup(410);p.combo=3;e.hp=0;const before=L.Particles.items.length;L.Combat.lethal(p,e,{kind:'combo',rare:true});assert(L.game.finalHold.momentum);assert(L.Particles.items.length>before);tick(12);assert.equal(L.Camera.mode,'final blow');assert(L.Camera.zoom>1)});
